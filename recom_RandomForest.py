...
class recom_rf():
    ...
    def downs_sample(self,size=10000):
        # down_sample the data, this help to speed up the training process
        ## adding time feature
        ...
        # use more recent data for better estimation
    ...
    def predict_rf(self,clf,X_test):
...
def main():
    df_rf = recom_rf()
    print('start')
    down_train = df_rf.downs_sample()
    print('Done downsizing')
    feature_train = df_rf.features_engineer(down_train)
    print('Done feature')
    clf,X_test,y_test = df_rf.train_rf(feature_train,feature_train['hotel_cluster'])
    print('Done training')
    y_pred = df_rf.predict_rf(clf,X_test)
    df_rf.write_output(y_pred,'rf')
